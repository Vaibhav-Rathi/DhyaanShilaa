...
const CreateCourse: React.FC = () => {
  ...
  const handleAddInstructor = (event: React.ChangeEvent<HTMLSelectElement>): void => {
  ...
  const handleRemoveInstructor = (id: string): void => {
  ...
  const updateLecture = (sectionIndex: number, lectureIndex: number, value: string): void => {
    const newSections = [...sections];
    newSections[sectionIndex].lectures[lectureIndex].name = value;
    setSections(newSections);
  };

  const handleContentsClick = (e: React.MouseEvent, sectionIndex: number, lectureIndex: number): void => {
    e.preventDefault();
  ...
  const addContentOption = (option: string, sectionIndex: number): void => {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>, sectionIndex: number, type: string): void => {
  ...
  const handleSaveEdit = (sectionIndex: number): void => {
    const newSections = [...sections];
  ...
  const handleEdit = (sectionIndex: number): void => {
    setEditingIndex(sectionIndex);
    setNewSectionName(sections[sectionIndex].name);
  };

  const handleDelete = (sectionIndex: number): void => {
  ...
  const handleTrailerUpload = (e: React.ChangeEvent<HTMLInputElement>): void => {
    const file = e.target.files?.[0];
    if (file) {
      setTrailer(file);
  ...
  const handleDeleteThumbnail = (): void => {
    setThumbnail(null);
    setThumbnailName("");
    setThumbnailPreview("");
  };
                          ...
                          <div className="mt-2">
                            <video
                              ...
                              onClick={() => {
                                ...
                                document
                                  .getElementById("trailer-upload")?.click();
                              }}
                            >
