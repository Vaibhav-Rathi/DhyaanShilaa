...
import { ArrowLeft } from "lucide-react"; // Make sure to import the ArrowLeft icon

// Define the message event type for YouTube API message event
...
export default function CoursePage() {
  ...
  const iframeRef = useRef<HTMLIFrameElement | null>(null); // Type for iframeRef
  const [progress, setProgress] = useState<number>(0); // Progress is a number
  ...
  // Function to get video progress
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      if (typeof event.data === "string") {
        try {
          const data: YouTubeMessage = JSON.parse(event.data); // Type the parsed data
        ...
        <nav className="text-xs space-y-4">
          ...
          <div>
            ...
            <p className="text-black mt-1 ml-6">
              <a
                href="https://www.testdome.com/tests/web-developer-online-test/16" // Replace with actual document URL
                target="_blank"
